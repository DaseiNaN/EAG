"""
Source code from : https://github.com/adbailey1/DepAudioNet_reproduction/blob/main/exp_run/models_pytorch.py
"""
import os
import sys
...
from src.ea import EmotionAnchorLayer
...
class DepAudioNetwithEA(nn.Module):
    def __init__(self, num_sentences, dropout, lstm_hidden_size, lstm_num_layers, lstm_bidirectional,
                 ea__feature_size, ea__anchor_size, ea__kernel_size, ea__stride, ea__dropout):
        super(DepAudioNetwithEA, self).__init__()
        self.conv = ConvBlock1d(in_channels=num_sentences,
                                out_channels=128,
                                kernel_size=3,
                                stride=1,
                                pad=1,
        ...
        self.fc = FullyConnected(in_channels=lstm_hidden_size * 2 + ea__anchor_size if lstm_bidirectional else lstm_hidden_size  + ea__anchor_size,
                                 out_channels=2,
                                 activation='sigmoid',
                                 normalisation=None)
        
        # Emotion Anchor Layer
        self.ea_layer = EmotionAnchorLayer(ea__feature_size, num_sentences, ea__anchor_size, ea__kernel_size, ea__stride, ea__dropout)

    def forward(self, x):
        batch_size, num_sentences, feat_dim = x.shape
        ea, x_pnn, emb_pos, emb_neu, emb_neg = self.ea_layer(x)
        ...
        x = torch.cat([x[:, -1, :].reshape(batch_size, -1), ea], dim = -1)
        x = self.fc(x)
        return x.squeeze(), x_pnn, emb_pos, emb_neu, emb_neg